...
func (obj *rfcObj) tcpMethod() (err error) {

	if _, err = io.ReadFull(obj.conn, obj.buffer[:2]); err != nil {
		return err
	}

	//fmt.Printf("tcpMethod 1- %x\n", obj.buffer[:2])
	mLen := int(obj.buffer[1])
	if mLen > 0 {
		if _, err = io.ReadFull(obj.conn, obj.buffer[:mLen]); err != nil {
			return err
		}
		//fmt.Printf("tcpMethod(%d) 2- %x\n", mLen, obj.buffer[:mLen])
	}
...
func (obj *rfcObj) request() (err error) {

	if _, err = io.ReadFull(obj.conn, obj.buffer[:4]); err != nil {
		return
	}

	//fmt.Printf("request 1- %x\n", obj.buffer[:4])
	...
	switch aType {
	case AtypDomainName:
		if _, err = io.ReadFull(obj.conn, obj.buffer[:1]); err != nil {
			return
		}
		addLen = int(obj.buffer[0])
	case AtypIPv4:
		addLen = net.IPv4len
		needParse = true
	case AtypIPv6:
		...
		needParse = true
	default:
		//err = fmt.Errorf("unkonw aType:%d", aType)   //no need to process this
		return
	}
	...
	//fmt.Printf("request 2 -len=%d, data= %x\n", addLen, obj.buffer[:addLen+2])
	...
	if needParse {
		fmt.Println("------>>>needParse=true=>", host, port)
		obj.Target = net.JoinHostPort(net.IP(host).String(), port)
	} else {
		fmt.Println("------>>>needParse=false=>", host, port)
...
Where:
          ...
          o  REP    Reply field:
             ...
             o  X’08’ MainAddress type not supported
...
func (obj *rfcObj) replies(addr string) (err error) {
	switch obj.cmd {
	case CmdConnect:
		if _, err = obj.conn.Write([]byte{5, 0, 0, 1, 0, 0, 0, 0, 0, 0}); err != nil {
			return
		}
	case CmdBind:
	...
	case CmdUDPAssociate:
		...
		if ip := net.ParseIP(host); ip != nil {
			...
			res = append(res, AtypDomainName, byte(len(host)))
			res = append(res, host...)
		}
		...
		res = append(res, byte(portNum>>8), byte(portNum))

		if _, err = obj.conn.Write(res); err != nil {
			return err
		}

		return InfoUDPAssociate
	}
	return nil
}
...
func (str *SimpleTunReader) GetTarget(conn net.Conn) string {
	...
	if err := obj.tcpMethod(); err != nil {
		fmt.Println("------>>>SimpleTunReader tcpMethod:=>", err)
		return ""
	}

	if err := obj.request(); err != nil {
		fmt.Println("------>>>SimpleTunReader request:=>", err)
		return ""
	}

	if err := obj.replies(conn.LocalAddr().String()); err != nil {
		fmt.Println("------>>>SimpleTunReader replies:=>", err)
		return ""
	}
...
func Socks5Target(conn net.Conn) string {
	...
	if err := obj.tcpMethod(); err != nil {
		fmt.Println("------>>> socks5 target tcp method:", err)
		return ""
	}

	if err := obj.request(); err != nil {
		fmt.Println("------>>> socks5 target request:", err)
		return ""
	}

	if err := obj.replies(conn.LocalAddr().String()); err != nil {
		fmt.Println("------>>> socks5 target replies:", err)
		return ""
	}
